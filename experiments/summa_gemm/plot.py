...
def plot3(y_label=None, show=True):
    # Get data
    mesh_sizes = [4, 8, 16, 32, 64, 128, 256]
    Mt = model.max_square_problem_size()
    e_hw = []
    e_sw = []
    for size in mesh_sizes:
        e_hw.append(model.e_summa_gemm(size, size, Mt, Mt, Mt, impl='hw'))
        e_sw.append(model.e_summa_gemm(size, size, Mt, Mt, Mt, impl='sw'))
    df = pd.DataFrame({
        'size': mesh_sizes,
        'e_hw': e_hw,
        'e_sw': e_sw,
    ...
    logL = logx - 0.5 * log_span
    logR = logx + 0.5 * log_span
    log_mid = 0.5 * (logL + logR)  # midpoint between left and right of each group
