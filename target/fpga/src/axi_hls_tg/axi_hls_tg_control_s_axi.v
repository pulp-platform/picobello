...
//        others - reserved
// 0x10 : Data signal of wide_port_offset
//        bit 31~0 - wide_port_offset[31:0] (Read/Write)
// 0x14 : Data signal of wide_port_offset
//        bit 31~0 - wide_port_offset[63:32] (Read/Write)
// 0x18 : reserved
// 0x1c : Data signal of narrow_port_offset
//        bit 31~0 - narrow_port_offset[31:0] (Read/Write)
// 0x20 : Data signal of narrow_port_offset
//        bit 31~0 - narrow_port_offset[63:32] (Read/Write)
// 0x24 : reserved
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_WIDE_PORT_OFFSET_DATA_0: begin
                    rdata <= int_wide_port_offset[31:0];
                end
                ADDR_WIDE_PORT_OFFSET_DATA_1: begin
                    rdata <= int_wide_port_offset[63:32];
                end
                ADDR_NARROW_PORT_OFFSET_DATA_0: begin
                    rdata <= int_narrow_port_offset[31:0];
                end
                ADDR_NARROW_PORT_OFFSET_DATA_1: begin
                    rdata <= int_narrow_port_offset[63:32];
                end
...
assign wide_port_offset   = int_wide_port_offset;
assign narrow_port_offset = int_narrow_port_offset;
assign traffic_dim        = int_traffic_dim;
assign compute_dim        = int_compute_dim;
assign traffic_id         = int_traffic_id;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
// int_wide_port_offset[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_wide_port_offset[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WIDE_PORT_OFFSET_DATA_0)
            int_wide_port_offset[31:0] <= (WDATA[31:0] & wmask) | (int_wide_port_offset[31:0] & ~wmask);
    end
end

// int_wide_port_offset[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_wide_port_offset[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WIDE_PORT_OFFSET_DATA_1)
            int_wide_port_offset[63:32] <= (WDATA[31:0] & wmask) | (int_wide_port_offset[63:32] & ~wmask);
    end
end

// int_narrow_port_offset[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_narrow_port_offset[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NARROW_PORT_OFFSET_DATA_0)
            int_narrow_port_offset[31:0] <= (WDATA[31:0] & wmask) | (int_narrow_port_offset[31:0] & ~wmask);
    end
end

// int_narrow_port_offset[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_narrow_port_offset[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NARROW_PORT_OFFSET_DATA_1)
            int_narrow_port_offset[63:32] <= (WDATA[31:0] & wmask) | (int_narrow_port_offset[63:32] & ~wmask);
    end
end

// int_traffic_dim[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_traffic_dim[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TRAFFIC_DIM_DATA_0)
            int_traffic_dim[31:0] <= (WDATA[31:0] & wmask) | (int_traffic_dim[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_compute_dim[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_COMPUTE_DIM_DATA_0)
            int_compute_dim[31:0] <= (WDATA[31:0] & wmask) | (int_compute_dim[31:0] & ~wmask);
    end
end
